...
* `6.5.2`- Splunk Enterprise base image [Dockerfile](https://github.com/ivanbrazza/docker-splunk/blob/master/enterprise/Dockerfile)
* `6.5.2-monitor`, `latest` - Splunk Enterprise with Docker Monitoring [Dockerfile](https://github.com/ivanbrazza/docker-itmonitoring/blob/master/enterprise/Dockerfile)
...
0. Enter the following command to pull the Splunk Enterprise version 6.5.2 image.<br>
   ...
   ```bash
   docker pull ivanbrazza/splunk
   ```
0. Run the Docker image.
   
   ```bash
   docker run -d -e "SPLUNK_START_ARGS=--accept-license" -e "SPLUNK_USER=root" -p "8000:8000" ivanbrazza/splunk
   ```
...
```bash
docker run --name splunk --hostname splunk -p 8000:8000 -d -e "SPLUNK_START_ARGS=--accept-license" ivanbrazza/splunk:6.5.2
```
### Start a Splunk Enterprise container and mount the necessary container volumes

```bash
docker run --name vsplunk -v /opt/splunk/etc -v /opt/splunk/var busybox
docker run --hostname splunk --name splunk --volumes-from=vsplunk -p 8000:8000 -d -e "SPLUNK_START_ARGS=--accept-license" ivanbrazza/splunk:6.5.2
```
...
```bash
docker run --name splunk --hostname splunk -p 8000:8000  -e "SPLUNK_START_ARGS=--accept-license" -v /opt/splunk/etc:/opt/splunk/etc -v /opt/splunk/var:/opt/splunk/var  ivanbrazza/splunk:6.5.2
```
...
The `ivanbrazza/splunk` image comes in several variants:

`ivanbrazza/splunk:6.5.2`
This is the default Splunk Enterprise image.

`ivanbrazza/splunk:6.5.2-monitor`
...
- `SPLUNK_NODE_TYPE='<master_node|indexer_cluster_peer|search_head_cluster_deployer|search_head_cluster_peer>'` - choose the type of node for this instance:
    - `master_node`: Enables the instance to be a [master node](http://docs.splunk.com/Documentation/Splunk/latest/Indexer/ConfiguremasterwithCLI).
    - `indexer_cluster_peer`: enables the instance to be an [indexer cluster peer](http://docs.splunk.com/Documentation/Splunk/latest/Indexer/ConfigurepeerswithCLI).
    - `search_head_cluster_deployer`: enables the instance to be a [deployer](http://docs.splunk.com/Documentation/Splunk/latest/DistSearch/SHCdeploymentoverview#2._Set_up_the_deployer) for a search head cluster.
    - `search_head_cluster_peer`: enables the instance to be a [search head cluster member](http://docs.splunk.com/Documentation/Splunk/latest/DistSearch/SHCdeploymentoverview#4._Initialize_cluster_members).
- `SPLUNK_REPLICATION_FACTOR='<positive integer>'` - specifies the [replication factor](http://docs.splunk.com/Splexicon:Replicationfactor) to use for indexer and search head clustering.
- `SPLUNK_SEARCH_FACTOR='<positive integer>'` - specifies the [search factor](http://docs.splunk.com/Splexicon:Searchfactor) to user for indexer clustering.
- `SPLUNK_SECRET='<secret_password>'` - specifies the secret key to use for indexer and search head clustering.
- `SPLUNK_CLUSTER_LABEL='<label_string>'` - specifies the label to use for the indexer or search head cluster. Useful for identifying the clusters in the DMC.
- `SPLUNK_MASTER_URI='<servername>:<port>'` - specifies the master node's DNS name and port (typically 8089) for the indexer peers joining the cluster.
- `SPLUNK_REPLICATION_PORT='<port>'` - the port to use for replication of buckets in an indexer cluster or search artifacts in a search head cluster.
- `SPLUNK_MGMT_URI='<servername>:<port>'` - specifies the DNS name and port (typically 8089) of the search head cluster member that's being deployed.
- `SPLUNK_DEPLOYER_URL='<servername>:<port>'` - specifies the DNS name and port (typically 8089) of the search head cluster deployer, used when deploying a search head cluster member.
- `SPLUNK_BOOTSTRAP_CAPTAIN='<true|false>'` - bootstrap the current node as the search head cluster captain and begin cluster deployment. You should *only* set this parameter to true when deploying the final search head in your cluster.
- `SPLUNK_SHCLUSTER_SERVER_LIST='<servername>:<port>,<servername>:<port>,...'` - a comma-separated list of the members in the search head cluster, only used when using the above parameter to bootstrap a cluster captain.
